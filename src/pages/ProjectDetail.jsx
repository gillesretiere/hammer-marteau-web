    ...
    <main className="min-h-screen bg-brand-bg pt-32 pb-24 transition-colors duration-300 text-left">
      <div className="max-w-7xl mx-auto px-6">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-4 gap-12 items-start text-left">
          ...
          {/* Cartouche Technique Latéral */}
          <aside className="lg:col-span-1 border border-brand-border bg-brand-surface/10 p-6 rounded-sm space-y-6 sticky top-32 font-mono text-xs text-left">
          ...
          {/* Corps éditorial — Aligné rigoureusement à gauche */}
          <article className="lg:col-span-3 prose prose-invert max-w-none font-mono text-xs md:text-sm text-left">
            ...
            <Markdown 
              ...
              components={{
                ...
                p: ({node, ...props}) => <p className="leading-relaxed text-brand-body opacity-80 mb-6 text-left" {...props} />,
                ul: ({node, ...props}) => <ul className="list-disc space-y-2 mb-6 pl-5 text-left" {...props} />,
                li: ({node, ...props}) => <li className="text-left" {...props} />,
                // Structure isolée pour intercepter et styliser proprement les blocs préformatés de code JSON
                pre: ({node, ...props}) => <pre className="bg-brand-surface/20 border border-brand-border p-5 rounded-sm font-mono text-[11px] text-brand-heading overflow-x-auto my-6 text-left" {...props} />,
                code: ({node, ...props}) => <code className="bg-brand-surface/30 px-1 py-0.5 rounded-sm font-mono text-xs text-brand-accent" {...props} />
              }}
            >
              {markdownContent}
            </Markdown>
